...
# DUAL ECM START
        ...
        # 强制设置一个明确的尺寸
        window_props.setSize(self.configs.window_width, self.configs.window_height)
        ...
        # 防御性代码：如果离屏 buffer 失败，借用主窗口
        if buffer is None:
            print(" [!] Warning: Offscreen buffer failed. Falling back to main window.")
            buffer = self.win

        # 创建第二个摄像头
        self.cam2 = self.makeCamera(buffer, clearDepth=1, lens=self.cam.node().getLens(), scene=self.render, camName='cam2')
        
        # 只有在成功创建了独立 buffer 的情况下才应用滤镜，否则会和主摄滤镜冲突
        if buffer != self.win:
