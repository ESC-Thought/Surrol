...
def build_ACT_model_and_optimizer(args_override):
    ...
    # 这里还通过 parents=[get_args_parser()] 继承了另一个参数解析器 get_args_parser()，
    # 以便获取该函数返回的参数集，通常是包含模型训练和评估相关参数的函数。
    parser = argparse.ArgumentParser('DETR training and evaluation script', parents=[get_args_parser()])
    # args将包含所有配置的参数及其值
    args = parser.parse_args()
    # 对于每一个键值对，将 args 对象中的对应属性（即键 k）设置为新值 v。
    # 这种方式允许用户通过 args_override 字典动态覆盖 args 中的默认参数值。
