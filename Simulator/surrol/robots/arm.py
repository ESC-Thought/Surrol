...
# 这些工具函数提供了 PyBullet 环境中的操作，
# 如获取关节位置、控制关节、正逆运动学计算等。
...
class Arm(object):
    ...
    def get_joint_number(self) -> int:
        ...
        return self.DoF
    # 用于 关节空间的增量移动，即相对于当前关节位置进行小范围调整。
    # 通过输入增量值来改变关节的位置，而不需要提供绝对目标值。
    ...
    def _check_joint_limits(self, abs_input: [list, np.ndarray]):
        ...
        if not np.all(np.bitwise_and(abs_input >= self.limits['lower'][:self.DoF],
            ...
            # sky
            # print("Lower limits:", self.limits['lower'][:self.DoF])
