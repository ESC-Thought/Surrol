...
class Data():
    ...
    def save_data(self, episode_idx):
        ...
        # save_path = f'/research/d1/gds/kjshi/IROS_SurRoL/collected_data/bi_peg_transfer_foveal_nodup_fixedgoal_masks_no_arm/image_action_qpos_{episode_idx}.pkl'
        save_path = f'/home/ztt/Desktop/KejianShi/IROS_SurRoL/collected_data/needle_pick_stereo_0806/image_action_qpos_{episode_idx}.pkl'
...
def goToGoal(env, last_obs, data_class):
    ...
    if args.env == "NeedleRegrasp-v0" or args.env == "BiPegTransfer-v2" or args.env == "BiPegTransfer-v3" :
    ...
    while time_step < min(env._max_episode_steps, args.steps) and not success:
        # action, rgb1, rgb2, mask_ori, mask_no_arm, mask_target, depth, robot_state, waypoint_idx, tip1, tip2, block, recorded_positions = env.get_oracle_action(obs)
        action,rgb1,rgb2,mask,depth,robot_state,waypoint_idx = env.get_oracle_action(obs)
        ...
        if args.env == "NeedlePick-v1":  
            ...
            if waypoint_idx == 5:
                for i in range(10):
                    ...
                    data_class.qpos[f'{str(waypoint_idx)}'].append(robot_state)
