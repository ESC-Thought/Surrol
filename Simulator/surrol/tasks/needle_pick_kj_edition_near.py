...
class NeedlePick(PsmEnv):
    ...
    # 机器人允许操作的空间范围
    WORKSPACE_LIMITS = ((0.20, 0.40), (-0.05, 0.05), (0.685, 0.7))  # reduce tip pad contact
    SCALING = 5.
    # 初始化关节位置
    QPOS_ECM = (0, 0.6, 0.2, 0)
    ...
    def _env_setup(self):
        ...
        self.ecm = Ecm((0.09, 0.0, 0.8124), 
                       scaling=self.SCALING)
        self.ecm.reset_joint(self.QPOS_ECM)
        self.STEREO=False  # TODO

        # robot
        workspace_limits = self.workspace_limits1
        # pos = (workspace_limits[0].mean() + (np.random.rand() - 0.3) * 0.1,  # TODO: scaling
        ...
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'needle/needle_40mm_RL.urdf'),
                            (workspace_limits[0].mean() + (np.random.rand() - 0.3) * 0.1,  # TODO: scaling
                             workspace_limits[1].mean() + (np.random.rand() - 0.5) * 0.1,
        ...
        pos_obj, _ = get_link_pose(self.obj_id, self.obj_link1)
        pos = (pos_obj[0]- 0.1 * 0.1,  # Add a small random offset near waypoint[1]
            pos_obj[1] - 0.1 * 0.1,
            pos_obj[2] + (-0.0007 + 0.0182) * self.SCALING)
        orn = (0.5, 0.5, -0.5, -0.5)
        joint_positions = self.psm1.inverse_kinematics((pos, orn), self.psm1.EEF_LINK_INDEX)
        self.psm1.reset_joint(joint_positions)
        self.block_gripper = False
    ...
    def _sample_goal(self) -> np.ndarray:
        ...
        goal = np.array([pos_obj[0]- 0.2 * 0.1,  # Add a small random offset near waypoint[1]
            pos_obj[1] - 0.2 * 0.1,
            pos_obj[2] + (-0.0007 + 0.0152) * self.SCALING])
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints[6] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102) * self.SCALING, yaw, -0.5])  # grasp        
        self._waypoints[7] = np.array([self.goal[0], self.goal[1],
