...
class PegTransfer(PsmEnv):
    ...
    def _env_setup(self):
        ...
        for i in self._pegs[7: 7 + num_blocks]:
        ...
        self.obj_id, self.obj_link1 = self._blocks[0], 0
...
#       self._pegs[0]: 2
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints.append(np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + 0.045 * self.SCALING, roll, pitch, yaw, 0.5]))  # above object
        self._waypoints.append(np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + (0.003 + 0.010) * self.SCALING, roll, pitch, yaw, 0.5]))  # approach
        self._waypoints.append(np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + (0.003 + 0.010) * self.SCALING, roll, pitch, yaw, -0.5]))  # grasp
        self._waypoints.append(np.array([pos_grasp[0] , pos_grasp[1],
        ...
        pos_peg = get_link_pose(self.obj_ids['fixed'][1],
                                self._pegs[self.obj_id - np.min(self._blocks) + 7])[0]  # 6 pegs
        pos_place = [self.goal[0] + pos_obj[0] - pos_peg[0],
                     self.goal[1] + pos_obj[1] - pos_peg[1], self._waypoints[0][2]]  # consider offset
        # pos_place = pos_place + np.dot(rot_mat,  np.array([-0.003, 0, 0]))
        self._waypoints.append(np.array([pos_place[0]+0.003, pos_place[1]+0.002, pos_place[2]-0.005, roll, pitch, yaw, -0.5])) # above goal
        self._waypoints.append(np.array([pos_place[0]+0.003, pos_place[1]+0.002, pos_place[2]-0.02, roll, pitch, yaw, -0.5])) # above goal
        self._waypoints.append(np.array([pos_place[0]+0.003, pos_place[1]+0.002, pos_place[2]-0.06, roll, pitch, yaw, -0.5]))  # release
        self._waypoints.append(np.array([pos_place[0]+0.003, pos_place[1]+0.002, pos_place[2]-0.06, roll, pitch, yaw, 0.5]))  # release
        self._waypoints.append(np.array([pos_place[0]+0.003, pos_place[1]+0.002, pos_place[2]-0.03, roll, pitch, yaw, 0.5]))  # release
