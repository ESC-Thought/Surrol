...
from Simulator.surrol.tasks.psm_env_rpy import PsmEnv, goal_distance
...
class PegTransfer(PsmEnv):
    ...
    QPOS_ECM = (0, 0.0, 0.1, 0)#(0, 0.5, 0.15, 0)#(0, 0.0, 0.1, 0) # (0,0.7,0.2, 0)
    ACTION_ECM_SIZE=3
    ...
    def _env_setup(self):
        ...
        self.ecm = Ecm((-0.05, 0.0, 0.8524),#(0.08, 0.0, 0.8524),#(-0.05, 0.0, 0.8524), #p.getQuaternionFromEuler((0, 30 / 180 * np.pi, 0)),
                       scaling=self.SCALING) # (0.15, 0.0, 0.8524)
    ...
    def _is_success(self, achieved_goal, desired_goal):
        ...
        result = np.logical_and(
            ...
            np.abs(achieved_goal[..., -1] - desired_goal[..., -1]) < 5e-3 * self.SCALING
...
#       self._pegs[0]: 2
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints = [None, None, None, None, None, None, None, None, None]  # six waypoints
        self._waypoints_done = [False, False, False, False, False, False, False, False, False]
        ...
        grasp_offset = np.array([-0.01, 0, 0])
        pos_grasp = pos_obj + np.dot(rot_mat, grasp_offset)
        ...
        self._waypoints[0] = np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + 0.045 * self.SCALING, roll, pitch, yaw, 0.5])  # above object
        self._waypoints[1] = np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + (0.003 + 0.0102) * self.SCALING, roll, pitch, yaw, 0.5])  # approach
        self._waypoints[2] = np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + (0.003 + 0.0102) * self.SCALING, roll, pitch, yaw, -0.5])  # grasp
        self._waypoints[3] = np.array([pos_grasp[0] , pos_grasp[1],
                                       pos_grasp[2] + 0.05 * self.SCALING, roll, pitch, yaw, -0.5])  # lift up
        ...
        self._waypoints[4] = np.array([pos_place[0], pos_place[1]+0.003, pos_place[2]-0.005, roll, pitch, yaw, -0.5]) # above goal
        self._waypoints[5] = np.array([pos_place[0], pos_place[1]+0.003, pos_place[2]-0.02, roll, pitch, yaw, -0.5]) # above goal
        self._waypoints[6] = np.array([pos_place[0], pos_place[1]+0.003, pos_place[2]-0.06, roll, pitch, yaw, -0.5])  # release
        self._waypoints[7] = np.array([pos_place[0], pos_place[1]+0.003, pos_place[2]-0.06, roll, pitch, yaw, 0.5])  # release
        self._waypoints[8] = np.array([pos_place[0], pos_place[1]+0.003, pos_place[2]-0.03, roll, pitch, yaw, 0.5])  # release
    ...
    def _get_obs(self) -> dict:
        ...
        tip, block, tip_pixel, block_pixel = self.save_cropped_tips_and_block(rgb1, tip_pos, block_pos, rgb1.shape[:2], crop_size=96, prefix='oracle')

        obs['images']['foveal_block'] = block
        obs['images']['foveal_tip'] = tip
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        robot_state = self._get_robot_state(idx=0)   # 7d
        output = self.ecm.render_image(stereo=self.STEREO)
        mask=output.mask1
        rgb1 = output.rgb1
        depth = output.depth1
        ...
        return action, rgb1, rgb2, None, None, mask, depth, robot_state, i, tip, block, [tip_pixel, block_pixel]
    ...
    def save_cropped_tips_and_block(self, rgb, tip1_pos, block_pos, img_shape, crop_size=64, save_dir='cropped_images', prefix='img'):
        """
        Save cropped images around the arm tip and the block by projecting their 3D positions to pixel coordinates.
        Args:
            rgb: RGB image (H, W, 3)
            tip1_pos: 3D position of the tool tip
        ...
        def crop_around_pixel(center, nafme):
            x_c, y_c = center
            half = crop_size // 2
            x1, x2 = max(0, x_c - half), min(rgb.shape[1], x_c + half)
            y1, y2 = max(0, y_c - half), min(rgb.shape[0], y_c + half)
        ...
        return tip1, block, tip1_pixel, block_pixel
