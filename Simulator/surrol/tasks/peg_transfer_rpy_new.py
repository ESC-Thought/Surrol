...
#       self._pegs[0]: 2
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints[2] = np.array([pos_grasp[0], pos_grasp[1],
                                       pos_grasp[2] + (0.000 + 0.01) * self.SCALING, roll, pitch, yaw, 0.5])  # approach
        self._waypoints[3] = np.array([pos_grasp[0], pos_grasp[1],
                                       pos_grasp[2] + (0.000 + 0.01) * self.SCALING, roll, pitch, yaw, -0.5])  # grasp
        ...
        self._waypoints[5] = np.array([pos_place[0]-0., pos_place[1], pos_place[2], roll, pitch, yaw, -0.5])  # above goal
        self._waypoints[6] = np.array([pos_place[0]-0., pos_place[1], pos_place[2]-0.05, roll, pitch, yaw, -0.5])  # release
        self._waypoints[7] = np.array([pos_place[0]-0., pos_place[1], pos_place[2]-0.05, roll, pitch, yaw, 0.5])  # release
        self._waypoints[8] = np.array([pos_place[0]-0., pos_place[1], pos_place[2]+0.02, roll, pitch, yaw, 0.5])  # release
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        # return action
        # sky
        return action, rgb1, rgb2, None, None, mask, depth, robot_state, i
