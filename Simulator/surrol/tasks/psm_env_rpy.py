...
from Simulator.surrol.robots.psm import Psm1, Psm2
from Simulator.surrol.utils.pybullet_utils import (
    get_link_pose,
    wrap_angle,
    reset_camera
)
from Simulator.surrol.utils.robotics import (
    get_euler_from_matrix,
    get_matrix_from_euler
)
from Simulator.surrol.const import ROOT_DIR_PATH, ASSET_DIR_PATH
...
class PsmEnv(SurRoLGoalEnv):
    ...
    """
    # sky
    # ACTION_SIZE = 5  # (dx, dy, dz, dyaw/dpitch, open/close) init
    ...
    def __init__(self,
                 render_mode=None, cid = -1, action_mode = 'yaw'):
        # workspace
        ...
        # sky
        PsmEnv.ACTION_MODE = action_mode
        if self.ACTION_MODE == 'rpy':
            PsmEnv.ACTION_SIZE = 7  # 
        elif self.ACTION_MODE == 'dq':
            PsmEnv.ACTION_SIZE = 6
        else:
            PsmEnv.ACTION_SIZE = 5

        # gripper
        self.block_gripper = True
        self._activated = -1
        self._max_constraint_id = 1
        super(PsmEnv, self).__init__(render_mode, cid, action_mode)
    ...
    def _env_setup(self,goal_plot=True):
        ...
        elif self.ACTION_MODE == 'rpy':
            # self.psm1_eul = np.array([np.deg2rad(-90), self.psm1_eul[1], self.psm1_eul[2]])
            pass
        elif self.ACTION_MODE == 'dq':
            pass
        else:
        ...
        if goal_plot:
            ...
            self.obj_ids['fixed'].append(obj_id)  # 0    
        # p.changeVisualShape(obj_id, -1, rgbaColor=(255/255, 0/255, 0/255, 0))
        # p.configureDebugVisualizer(lightPosition=(10.0, 10.0, 10.0))
        # print(f'goal:{obj_id}')
    ...
    def _get_robot_state(self, idx: int) -> np.ndarray:
        ...
        pose_world = psm.pose_rcm2world(psm.get_current_position(), 'tuple')  # return: pose in the transformed frame
        # pose_world = psm.pose_rcm2world(psm.get_current_position())  
        jaw_angle = psm.get_current_jaw_position() # 获取两个夹爪实际开合角度
    ...
    def _get_obs(self) -> dict:
        ...
        if self.has_object:
            ...
            # rotations
            waypoint_rot = np.array(p.getEulerFromQuaternion(orn))  # 3
    ...
    def _set_action(self, action: np.ndarray):
        ...
        if self.ACTION_MODE == 'yaw':
            ...
            action[3] *= np.deg2rad(20)  # roll, limit maximum change in rotation
            roll = wrap_angle(rot[0] + action[3])

            action[4] *= np.deg2rad(20)  # pitch, limit maximum change in rotation
            pitch = np.clip(wrap_angle(rot[1] + action[4]), np.deg2rad(-90), np.deg2rad(90))

            action[5] *= np.deg2rad(20)  # yaw, limit maximum change in rotation
            yaw = wrap_angle(rot[2] + action[5])

            rot = (roll, pitch, yaw)  # only change yaw
            pose_world[:3, :3] = get_matrix_from_euler(rot)
            action_rcm = self.psm1.pose_world2rcm(pose_world)
            # time1 = time.time()
            self.psm1.move(action_rcm)
            ...
            self.psm1.dmove_joint(action[:-1])
        else:    
            raise NotImplementedError
...
class PsmsEnv(PsmEnv):
    ...
    """
    ACTION_SIZE = 7 * 2  # (dx, dy, dz, dyaw/dpitch, open/close) * 2
    ACTION_MODE = 'yaw'
    DISTANCE_THRESHOLD = 0.005
    POSE_PSM1 = ((0.05, 0.24, 0.8524), (0, 0, -(90 + 20) / 180 * np.pi))
    QPOS_PSM1 = (0, 0, 0.10, 0, 0, 0)
    ...
    def __init__(self,
        ...
        # sky
        PsmsEnv.ACTION_MODE = action_mode
        if self.ACTION_MODE == 'rpy' or self.ACTION_MODE == 'pitch':
            print("ABABABABA")
            PsmsEnv.ACTION_SIZE = 14
        elif self.ACTION_MODE == 'dq':
            PsmsEnv.ACTION_SIZE = 12
        else:
            PsmsEnv.ACTION_SIZE = 10
        super(PsmsEnv, self).__init__(render_mode, cid, action_mode)
    ...
    def _set_action(self, action: np.ndarray):
        ...
        *2 for PSM1 [0: 5] and PSM2 [5: 10]
        ...
        # print("real action size:", PsmsEnv.ACTION_SIZE)
        # print(len(action))
        ...
        for i, psm in enumerate((self.psm1, self.psm2)):
            ...
            if self.ACTION_MODE == 'yaw':
                ...
                action[idx + 4] *= np.deg2rad(15)  # limit maximum change in rotation
                pitch = np.clip(wrap_angle(rot[1] + action[idx + 4]), np.deg2rad(-90), np.deg2rad(90))    # attention: original: +3
                rot = (psm_eul[0], pitch, psm_eul[2])  # only change pitch
                # print(f"AAAAAAAAAAAA: {psm_eul[0]}, {psm_eul[2]}")
            elif self.ACTION_MODE == 'rpy':
        ...
        #                                  pos_mid2[2], roll2, pitch2, yaw2, 0.5]))  # above object
                action[idx + 3] *= np.deg2rad(20)
                roll = wrap_angle(rot[0] + action[idx + 3])
                action[idx + 4] *= np.deg2rad(20)
                pitch = np.clip(wrap_angle(rot[1] + action[idx + 4]), np.deg2rad(-90), np.deg2rad(90))
                action[idx + 5] *= np.deg2rad(20)
                yaw = wrap_angle(rot[2] + action[idx + 5])
                rot = (roll, pitch, yaw)
            # else:
                # raise NotImplementedError
            pose_world[:3, :3] = get_matrix_from_euler(rot)
