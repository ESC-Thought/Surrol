...
from Simulator.surrol.utils.batch import Batch
INF = np.inf
PI = np.pi
...
def reset_camera(yaw=50.0, pitch=-35.0, dist=5.0, target=(0.0, 0.0, 0.0), scaling=1.0):
...
def render_image(width, height, view_matrix, proj_matrix, shadow=1, stereo=False, scaling=None):
    ...
    if stereo:
        ...
        cam2_2_cam1[0, 3] = -0.004214 * 5
        cam2_view_matrix = np.matmul(cam2_2_cam1, np.array(view_matrix).reshape(4,4).T).T.flatten()


        # plt.imsave('/home/kejianshi/Desktop/Surgical_Robot/SurRoLv2/surrol/data/rgb_array1.png', rgb_array)
        (_, _, px2, depth2, mask2) = p.getCameraImage(width=width,
                                            height=height,
                                            viewMatrix=cam2_view_matrix,
                                            projectionMatrix=proj_matrix,
                                            shadow=shadow,
                                            lightDirection=(10, 0, 10),
                                            renderer=renderer)
        ...
        # plt.imsave('/home/kejianshi/Desktop/Surgical_Robot/SurRoLv2/surrol/data/rgb_array2.png', rgb_array2)
        return Batch(rgb1=rgb_array, mask1=mask, depth1=depth, rgb2=rgb_array2, mask2=mask2, depth2=depth2)
    else:

        return Batch(rgb1=rgb_array, mask1=mask, depth1=depth)
