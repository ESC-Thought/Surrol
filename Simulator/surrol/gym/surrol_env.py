...
class SurRoLEnv(gym.Env):
    ...
    def __init__(self, render_mode: str = None, cid: int = -1, action_mode: str = None):
        ...
        if render_mode == 'human':
            ...
            # Load EGL only when explicitly requested; some headless machines
            # fail at reset if the plugin is forced in DIRECT mode.
            use_egl = os.environ.get("SURROL_USE_EGL", "0").lower() in ("1", "true", "yes", "on")
            if use_egl:
                egl = pkgutil.get_loader('eglRenderer')
                if egl is None:
                    raise RuntimeError("SURROL_USE_EGL=1 but PyBullet eglRenderer plugin was not found.")
        ...
        obs = self._get_obs()
        # print("ababababbaba:",self.action_size)
    ...
    def step(self, action: np.ndarray):
        action = np.zeros(self.action_size) if action is None else action
        # action should have a shape of (action_size, )
        # print("eeeeeeeeeeeeeeeeeee:",len(action))
        ...
        # print("goalaaaaa:",obs['achieved_goal'])
        done = self._is_success(obs['achieved_goal'], self.goal).astype(bool)
        info = {
