...
from Simulator.surrol.gui.scene import Scene, GymEnvScene
from Simulator.surrol.gui.application import Application, ApplicationConfig
from Simulator.surrol.tasks.ecm_misorient import MisOrient
from Simulator.surrol.tasks.ecm_reach import ECMReach
from Simulator.surrol.tasks.ecm_active_track import ActiveTrack
from Simulator.surrol.tasks.ecm_static_track import StaticTrack
from Simulator.surrol.tasks.gauze_retrieve import GauzeRetrieve
from Simulator.surrol.tasks.needle_reach import NeedleReach
from Simulator.surrol.tasks.needle_pick import NeedlePick
from Simulator.surrol.tasks.peg_board import PegBoard
from Simulator.surrol.tasks.peg_board_bimanual import BiPegBoard
from Simulator.surrol.tasks.peg_transfer import PegTransfer 
from Simulator.surrol.tasks.peg_transfer_RL import PegTransferRL
from Simulator.surrol.tasks.needle_regrasp_bimanual import NeedleRegrasp
from Simulator.surrol.tasks.peg_transfer_bimanual import BiPegTransfer
from Simulator.surrol.tasks.pick_and_place import PickAndPlace
from Simulator.surrol.tasks.match_board import MatchBoard
from Simulator.surrol.tasks.match_board_ii import MatchBoardII 

from Simulator.surrol.tasks.needle_the_rings import NeedleRings
# from surrol.tasks.match_board_ii import BiMatchBoard
from Simulator.surrol.tasks.ecm_env import EcmEnv, goal_distance,reset_camera
...
from Simulator.surrol.utils.pybullet_utils import step
...
class SurgicalSimulator(SurgicalSimulatorBase):
    def __init__(self, env_type, env_params,id=None,demo=None):
        ...
        # Change keyboard bindings for recording to use different keys
        self.app.accept('p-up', self.start_recording)  # Changed from 's-up' to 'p-up'
        self.app.accept('o-up', self.stop_recording)   # Changed from 't-up' to 'o-up'
        ...
        if self.demo == None:
            ...
            if task.time - self.time > 1 / 240.0:
                ...
                (_, _, px, depth, mask) = p.getCameraImage(
                                                            ...
                                                            projectionMatrix=self.env._proj_matrix, renderer=p.ER_BULLET_HARDWARE_OPENGL)
                ...
                cv2.imshow('rgb', cv2.cvtColor(rgb_array, cv2.COLOR_RGB2BGR))
                cv2.waitKey(1)
    ...
    def before_simulation_step(self):
        # First handle recording if active
        if self.is_recording:
            # Get current robot state and observation
            obs = self.env._get_obs()
            rgb = obs['images']['rgb1']
            state = obs['observation'][:7]
            # Record current state
            self.recorded_states.append(state)
            self.recorded_observations.append(rgb)
    ...
    def start_recording(self):
        """Start recording robot states and observations"""
        if not self.is_recording:
            ...
            print("Recording robot states and observations...")
    ...
    def stop_recording(self):
        ...
        if self.is_recording:
            ...
            save_dir = f"robot_recordings/{timestamp}"
            os.makedirs(save_dir, exist_ok=True)
            
            # Save recorded data
            np.save(os.path.join(save_dir, "states.npy"), np.array(self.recorded_states))
            np.save(os.path.join(save_dir, "observations.npy"), np.array(self.recorded_observations))
            
            print("\n=== STOP RECORDING ===")
            print(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"Saved {len(self.recorded_states)} frames to {save_dir}")
