...
class PolicyEvaluator:
    ...
    def collect_correction_data(self, episode_num):
        ...
        while True:  # Loop to allow episode resets
            ...
            act_corrected = []
            obs_list = []
            state_before_corrections = []  # Store robot state before corrections
            state_after_corrections = []   # Store robot state after corrections
            ...
            while not success and episode_length < 200:
                ...
                if needs_correction:
                    ...
                    for control_action in self.gui_control_robot(policy_action, accumulated_corrections=[], episode_num=episode_num):
                        ...
                        obs, reward, done, info = self.env.step(control_action)
                    ...
                    # No correction needed, apply policy action
            ...
            if success:
                ...
                h5_path = os.path.join(episode_dir, f'episode_{episode_num:03d}.h5')
    ...
    def move_x_positive(self, control_data, status_label, apply_action):
        """Move in positive X direction or adjust roll based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            
            # Apply position change
            control_data['action'][0] += control_data['step_size']
            
            # Restore original orientation
            control_data['action'][3:6] = original_rpy_right
            control_data['action'][10:13] = original_rpy_left
            
            status_label.config(text=f"Moved X+: {control_data['action'][0]:.3f}")
        else:
            # Adjust roll (right gripper)
            control_data['action'][3] += control_data['step_size']
            status_label.config(text=f"Increased Roll: {control_data['action'][3]:.3f}")
        
        apply_action()

    def move_x_negative(self, control_data, status_label, apply_action):
        """Move in negative X direction or adjust roll based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust roll (right gripper)
            control_data['action'][3] -= control_data['step_size']
            status_label.config(text=f"Decreased Roll: {control_data['action'][3]:.3f}")
        
        apply_action()

    def move_y_positive(self, control_data, status_label, apply_action):
        """Move in positive Y direction or adjust pitch based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust pitch (right gripper)
            control_data['action'][4] += control_data['step_size']
            status_label.config(text=f"Increased Pitch: {control_data['action'][4]:.3f}")
        
        apply_action()

    def move_y_negative(self, control_data, status_label, apply_action):
        """Move in negative Y direction or adjust pitch based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust pitch (right gripper)
            control_data['action'][4] -= control_data['step_size']
            status_label.config(text=f"Decreased Pitch: {control_data['action'][4]:.3f}")
        
        apply_action()

    def move_z_positive(self, control_data, status_label, apply_action):
        """Move in positive Z direction or adjust yaw based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust yaw (right gripper)
            control_data['action'][5] += control_data['step_size']
            status_label.config(text=f"Increased Yaw: {control_data['action'][5]:.3f}")
        
        apply_action()

    def move_z_negative(self, control_data, status_label, apply_action):
        """Move in negative Z direction or adjust yaw based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust yaw (right gripper)
            control_data['action'][5] -= control_data['step_size']
            status_label.config(text=f"Decreased Yaw: {control_data['action'][5]:.3f}")
        
        apply_action()

    def move_x2_positive(self, control_data, status_label, apply_action):
        """Move left gripper in positive X direction or adjust roll based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust roll (left gripper)
            control_data['action'][10] += control_data['step_size']
    ...
    def move_x2_negative(self, control_data, status_label, apply_action):
        """Move left gripper in negative X direction or adjust roll based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
    ...
    def move_y2_positive(self, control_data, status_label, apply_action):
        """Move left gripper in positive Y direction or adjust pitch based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust pitch (left gripper)
            control_data['action'][11] += control_data['step_size']
    ...
    def move_y2_negative(self, control_data, status_label, apply_action):
        """Move left gripper in negative Y direction or adjust pitch based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
    ...
    def move_z2_positive(self, control_data, status_label, apply_action):
        """Move left gripper in positive Z direction or adjust yaw based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
            ...
            # Adjust yaw (left gripper)
            control_data['action'][12] += control_data['step_size']
    ...
    def move_z2_negative(self, control_data, status_label, apply_action):
        """Move left gripper in negative Z direction or adjust yaw based on mode"""
        if control_data['control_mode'] == 'position':
            # Store original orientation
            original_rpy_right = control_data['action'][3:6].copy()
            original_rpy_left = control_data['action'][10:13].copy()
