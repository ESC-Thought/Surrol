...
from surrol.tasks.peg_transfer_bimanual_new_add_foveal import BiPegTransfer
...
class PolicyEvaluator:
    def __init__(self):
        ...
        self.video_dir = os.path.expanduser('~/Desktop/Surgical_Robot/Surrol_Related/IROS_SurRoL/rl/act-main-3/experiments/eval/whole_policy/0707-stereo-100_correction/')
        print("\n=== Initialization ===")
        ...
        self.model_paths = {
            'whole': {
                'dir': '/home/kejianshi/Desktop/Surgical_Robot/Surrol_Related/IROS_SurRoL/rl/act-main-3/experiments/ACT_BIPEG_100',
    ...
    def interactive_control_loop(self, obs, episode_length=0):
        ...
        while not success and episode_length < 200:
            ...
            action_yaw[0:3] = action[0:3]
            action_yaw[3] = action[5]
            action_yaw[4] = action[6]

            action_yaw[5:7] = action[8:10]
            action_yaw[8] = action[12]
            action_yaw[9] = action[13]
            # Apply policy action
    ...
    def collect_correction_data(self, episode_num):
        ...
        while True:  # Loop to allow episode resets
            ...
            data = {
                ...
                "policy_action": [],              # shape (L, 14)
                "policy_obs": [],                 # shape (L, 25)
                "post_intervention_eef_pose": [], # shape (L, 6)
                "pre_intervention_eef_pose": [],  # shape (L, 6)
                'correction_action': [],
                'images': []
            }
            ...
            while not success and episode_length < 200:
                ...
                data["policy_obs"].append(observation)  # Store the 25-dim observation vector
                ##############################
                ###### apply act policy ######
                ##############################
                ...
                # Apply policy action
                action_yaw = np.zeros(10)
                action_yaw[0:3] = policy_action[0:3]
                action_yaw[3] = policy_action[5]
                action_yaw[4] = policy_action[6]
                action_yaw[5:8] = policy_action[7:10]
                action_yaw[8] = policy_action[12]
                action_yaw[9] = policy_action[13]
                obs, reward, done, info = self.env.step(action_yaw)
                ...
                if needs_correction:
                    ...
                    for control_action in self.gui_control_robot(policy_action, accumulated_corrections=[], episode_num=episode_num):
                        ...
                        action_yaw = np.zeros(10)
                        action_yaw[0:3] = control_action[0:3]
                        action_yaw[3] = control_action[5]
                        action_yaw[4] = control_action[6]
                        action_yaw[5:8] = control_action[7:10]
                        action_yaw[8] = control_action[12]
                        action_yaw[9] = control_action[13]
                        final_obs, reward, done, info = self.env.step(action_yaw)
                        total_correction += action_yaw
            ...
            if success:
                ...
                try:
                    with h5py.File(h5_path, 'w') as f:
                        ...
                        for key in data:
                            if key == "images":
                                # Reorganize images by camera type
                                images_grp = f.create_group("images")
                                
                                # First, get all camera types from the first frame
                                camera_types = list(data["images"][0].keys())
                                
                                # For each camera type, create a dataset with all frames
                                for cam_type in camera_types:
                                    # Stack all frames for this camera type
                                    frames = np.stack([img_dict[cam_type] for img_dict in data["images"]], axis=0)
                                    # Save as a single dataset with shape (sequence_length, height, width, channels)
                                    images_grp.create_dataset(cam_type, data=frames)
                                    print(f"Saved {cam_type} images with shape {frames.shape}")
                            elif key == "policy_obs":
    ...
    def move_z_positive(self, control_data, status_label, apply_action):
        ...
        if control_data['control_mode'] == 'position':
            ...
            # In orientation mode, adjust yaw for left arm
            control_data['action'][8] += control_data['step_size']  # yaw for left arm
            status_label.config(text=f"Increased Left Yaw: {control_data['action'][8]:.3f}")
    ...
    def move_z_negative(self, control_data, status_label, apply_action):
        ...
        if control_data['control_mode'] == 'position':
            ...
            # In orientation mode, adjust yaw for left arm
            control_data['action'][8] -= control_data['step_size']  # yaw for left arm
            status_label.config(text=f"Decreased Left Yaw: {control_data['action'][8]:.3f}")
        
        apply_action()

    def move_x2_positive(self, control_data, status_label, apply_action):
        # Remove - roll control not used in yaw mode
        pass
    ...
    def move_y2_positive(self, control_data, status_label, apply_action):
        # Remove - pitch control not used in yaw mode
        pass
    ...
    def move_z2_positive(self, control_data, status_label, apply_action):
        ...
        if control_data['control_mode'] == 'position':
            ...
            # Apply position change to left arm Z
            control_data['action'][7] += control_data['step_size']  # Index 7 is left arm Z in yaw mode
            ...
            status_label.config(text=f"Moved Left Z+: {control_data['action'][7]:.3f}")
        else:
            # Adjust yaw for left arm
            control_data['action'][8] += control_data['step_size']  # Index 8 is left arm yaw
            status_label.config(text=f"Increased Left Yaw: {control_data['action'][8]:.3f}")
    ...
    def move_z2_negative(self, control_data, status_label, apply_action):
        ...
        if control_data['control_mode'] == 'position':
            ...
            # Apply position change
            control_data['action'][7] -= control_data['step_size']  # Changed from 9 to 7 for yaw mode
            ...
            # Adjust yaw for left arm
            control_data['action'][8] -= control_data['step_size']  # Changed from 12 to 8 for yaw mode
            status_label.config(text=f"Decreased Left Yaw: {control_data['action'][8]:.3f}")
...
def main():
    ...
    if evaluator.mode == 'whole':
        ...
        for i in range(total_trials):
            success_cnt = evaluator.evaluate_whole_policy(i+100, success_cnt=success_cnt)
