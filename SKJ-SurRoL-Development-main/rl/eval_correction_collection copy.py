...
class PolicyEvaluator:
    def __init__(self):
        ...
        self.video_dir = '/home/kejianshi/Desktop/Surgical_Robot/Surrol_Related/IROS_SurRoL/rl/act-main-3/experiments/eval/whole_policy/0425-stereo-100_correction_2/'
        ...
        while True:  # Loop to allow episode resets
            ...
            while not success and episode_length < 200:
                ...
                if needs_correction:
                    ...
                    act_corrected.append(np.concatenate([np.array(correction1), np.array(correction2)], axis=0))
                else:
            ...
            if success:
                ...
                with h5py.File(h5_path, 'w') as f:
                    ...
                    policy_grp = f.create_group('policy_actions')
                    correction_grp = f.create_group('corrections')
                    obs_grp = f.create_group('observations')
                    
                    # Save policy actions
                    for i, action in enumerate(act_policy):
                        policy_grp.create_dataset(f'action_{i}', data=action)
                    
                    # Save correction indicators
                    correction_grp.create_dataset('indicators', data=np.array(correction_indicator))
                    
                    # Save corrected actions
                    if act_corrected:
                        corrected_grp = f.create_group('corrected_actions')
                        for i, action in enumerate(act_corrected):
                            corrected_grp.create_dataset(f'corrected_action_{i}', data=action)
                    ...
                    for i, ob in enumerate(obs_list):
                        ...
                        for key, value in ob.items():
                            if isinstance(value, np.ndarray):
                                ...
                                nested_grp = obs_step.create_group(key)
                                for k, v in value.items():
                                    if isinstance(v, np.ndarray):
                                        nested_grp.create_dataset(k, data=v)
                    
                    # Save metadata
                    f.attrs['episode_num'] = episode_num
            ...
            # Return from the function if we didn't reset
