...
class Data():
    ...
    def save_data(self, episode_idx):
        ...
        """
        save_path = f'/research/d1/gds/kjshi/IROS_SurRoL/collected_data/bi_peg_hdf5/image_action_qpos_{episode_idx}.h5'
        
        with h5py.File(save_path, 'w') as f:
            # Create groups for different data types
            for waypoint_idx in self.images.keys():
                grp = f.create_group(waypoint_idx)
                
                # Save images (RGB arrays)
                images = np.array(self.images[waypoint_idx])
                grp.create_dataset('images', data=images, compression='gzip', compression_opts=9)
                
                # Save actions
                grp.create_dataset('actions', data=np.array(self.actions[waypoint_idx]), compression='gzip')
                
                # Save joint differences
                grp.create_dataset('dq_actions', data=np.array(self.dq[waypoint_idx]), compression='gzip')
                
                # Save joint positions
                grp.create_dataset('qpos', data=np.array(self.qpos[waypoint_idx]), compression='gzip')
                
                # Save masks
                grp.create_dataset('masks_target', data=np.array(self.masks_target[waypoint_idx]), compression='gzip')
                
                # Save depths if they exist
                if self.depths.get(waypoint_idx):
                    grp.create_dataset('depths', data=np.array(self.depths[waypoint_idx]), compression='gzip')
