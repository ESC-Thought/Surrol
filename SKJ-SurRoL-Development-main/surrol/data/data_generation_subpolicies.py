...
class Data():
    ...
    def save_data(self, episode_idx):
        ...
        save_path = f'/research/d1/gds/kjshi/IROS_SurRoL/collected_data/bi_peg_sub_data/handover_subs/image_action_qpos_{episode_idx}.pkl'
...
def main():
    ...
    ### loading policy ###
    act_class1 = ACT_Policy('ACT')
    act_class1.load_ckpt('/research/d1/gds/kjshi/IROS_SurRoL/rl/act-main-3/experiments/sub_policies/grasp/20250227_1538_bi_peg_transfer_grasp_sub_clean', 'policy_epoch_1926_seed_0.ckpt')
    ...
    while success_cnt < 200:
        ...
        goToGoal(env, obs, data_class, act_class1)
        cnt += 1
...
def goToGoal(env, last_obs, data_class, act_class1):
    global success_cnt
    ...
    while time_step < min(env._max_episode_steps, args.steps) and not success:
        action, rgb1, rgb2, mask_ori, mask_target, depth, robot_joint_state, waypoint_idx = env.get_oracle_action(obs)
        pos_obj = obs['observation'][14:17]
        if pos_obj[2] < 3.65:
            action = act_class1.get_action(obs, time_step)
            waypoint_idx = -1 # waypoint -1 means policy actions
        ...
        if isinstance(obs, dict) and waypoint_idx == 5:
